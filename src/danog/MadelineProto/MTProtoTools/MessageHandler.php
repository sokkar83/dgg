...
trait MessageHandler
{
    ...
    public function send_message($message_data, $content_related, $aargs = [])
    {
        ...
        $this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages[$message_id] = []; //'content' => $this->deserialize($message_data, ['type' => '', 'datacenter' => ''])];
        ...
        if ($this->datacenter->sockets[$aargs['datacenter']]->temp_auth_key['auth_key'] === null || $this->datacenter->sockets[$aargs['datacenter']]->temp_auth_key['server_salt'] === null) {
            ...
            if (count($this->datacenter->sockets[$aargs['datacenter']]->object_queue) > 1) {
                ...
                //$this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages[$message_id] = ['content' => $this->deserialize($message_content, ['type' => '', 'datacenter' => '']), 'seq_no' => $seq_no];
